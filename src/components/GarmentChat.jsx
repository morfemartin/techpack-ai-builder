...
const SYSTEM_PROMPT =
  ...
  "Tu objetivo es juntar: 1) que prenda es, 2) sus piezas de construccion con valores concretos - no preguntes " +
  '"que piezas tiene" en general, pregunta una por una con ejemplos: "tu polo lleva botones, cuantos? son personalizados?", ' +
  '"tiene cola de pato (bajo curvo) o bajo recto?" - 3) si tiene diseno o bordado, cuantos, y colores Pantone si los sabe.\n\n' +
  "No sigas un guion fijo por tipo de prenda - usa tu conocimiento de construccion de indumentaria para preguntar lo " +
  "relevante a la prenda que te digan, con el mismo estilo que esos ejemplos.\n\n" +
  ...
  '"designs": "resumen breve de lo que dijeron sobre disenos/bordado/colores, string vacio si no hablaron de eso todavia"}, ' +
  '"done": false}\n\n' +
  '"draft" es SIEMPRE el estado acumulado completo hasta ahora, no solo lo nuevo de este turno - repeti las piezas ' +
  "ya conocidas ademas de la nueva. \"done\" pasa a true recien cuando ya preguntaste las piezas principales y si hay " +
  "diseno/bordado, y el usuario no tiene mas que agregar - en ese ultimo turno, \"reply\" es un mensaje de cierre, no una pregunta."
...
export function GarmentChat({ onComplete }) {
  ...
  async function send() {
    ...
    setError(null)
    try {
      const apiMessages = [{ role: "system", content: SYSTEM_PROMPT }, ...nextHistory]
      ...
      <div style={{ width: 240, flexShrink: 0, border: hair, background: C.white.hex, padding: space(3) }}>
        ...
        {draft && (
          <div>
            ...
            {draft.designs && (
              <div style={{ marginTop: space(2), paddingTop: space(2), borderTop: "1px solid #E6E8EC", fontSize: type.size.xs, color: C.ink.hex }}>{draft.designs}</div>
            )}
          </div>
        )}
      </div>
